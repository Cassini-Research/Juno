...
enum JunoUserDefaults {
    ...
    /// Live transcriptions in the HUD. When ON (default on eligible Macs)
    /// the floating HUD shows the partial transcript as you speak (full
    /// island). When OFF the HUD collapses to a tiny waveform pill, and the
    /// engine skips per-utterance preview-lane decoding to save CPU/GPU. The
    /// model is still installed and the resident streaming-preview service
    /// stays warm — we only suppress the per-utterance `decode(...)` call.
